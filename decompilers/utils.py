...
def send_result(result: Dict[str, Any], api_base_url: str) -> Response:
    logging.info(
        f'Seding result to API for sample with sha1={result["statistics"]["sha1"]}.')
...
def get_sample(seaweedfs_file_id: str) -> bytes:
    ...
    logging.info(
        f'Downloaded sample with seaweedfs_file_id: {seaweedfs_file_id}')
    return sample
...
def shutil_compression_algorithm_to_extnesion(shutil_algorithm: str) -> str:
    shutil_algorithm_to_extension = {
        'zip': 'zip', 'gztar': 'tar.gz', 'bztar': 'tar.bz2', 'xztar': 'tar.xz'}
